...
dependencies {
    ...
    if (!platform.isFabric) {
        ...
        if (!platform.isNeoForge) {
            // NeoForge bundles Mixin itself; shading it in would cause conflicts
    ...
    if (platform.isNeoForge) {
        // NeoForge bundles MixinExtras; declare common artifact for compile-time annotations
    ...
    if (irisVersion != null && platform.isFabric) {
...
tasks.jar {
    ...
    if (platform.isNeoForge) {
        // Exclude Fabric metadata; NeoForge uses META-INF/neoforge.mods.toml instead
...
val bundleJar by tasks.registering(com.github.jengelman.gradle.plugins.shadow.tasks.ShadowJar::class) {
    ...
    // Relocated third-party service files reference classes no longer at their original names,
