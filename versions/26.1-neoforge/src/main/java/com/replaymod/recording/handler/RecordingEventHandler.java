...
//#else
//$$ import net.minecraft.network.packet.s2c.play.PositionMoveRotationS2CPacket;
//#endif
...
public class RecordingEventHandler extends EventRegistrations {
    ...
    public void spawnRecordingPlayer() {
        try {
            ...
            //#elseif MC>=12002
            //$$ packetListener.save(new ClientboundAddEntityPacket(player));
            //#else
            //$$ packetListener.save(new PlayerSpawnS2CPacket(player));
            //#endif
            //#if MC>=11903
            packetListener.save(new ClientboundSetEntityDataPacket(player.getId(), player.getEntityData().packDirty()));
            //#elseif MC>=11500
            //$$ packetListener.save(new ClientboundSetEntityDataPacket(player.getId(), player.getDataTracker(), true));
            //#endif
    ...
    private void onPlayerTick() {
        ...
        try {
            ...
            if (force || Math.abs(dx) > maxRelDist || Math.abs(dy) > maxRelDist || Math.abs(dz) > maxRelDist) {
                ...
                //#elseif MC>=10800
                //$$ packet = new PositionMoveRotationS2CPacket(player);
                //#else
                ...
                //$$         Mth.floor_double(player.boundingBox.minY * 32),
		...
		lastEffects = found;
            ...
            //#endif
                if (!ItemStack.matches(playerItems.get(index), stack)) {
                    ...
                    //#else
                    //$$ packetListener.save(new ClientboundSetEquipmentPacket(player.getEntityId(), slot, stack));
                    //#endif
                }
            }
