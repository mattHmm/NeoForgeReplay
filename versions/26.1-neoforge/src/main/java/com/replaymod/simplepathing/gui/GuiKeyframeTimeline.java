...
public class GuiKeyframeTimeline extends AbstractGuiTimeline<GuiKeyframeTimeline> implements Draggable {
    ...
    protected void drawTimelineCursor(GuiRenderer renderer, ReadableDimension size) {
        ...
        if (!linesRenderState.lines.isEmpty()) {
            ...
            CrossFrameResourcePool pool = ((GameRendererAccessor) mc.gameRenderer).getPool();
            TimeTimelineLinesRenderer linesRenderer = pool.acquire(TimeTimelineLinesRenderer.FACTORY);
            pushScissorState();
            setScissorDisabled();
            linesRenderer.prepare(linesRenderState, ((DrawContextAccessor) renderer.getContext()).getState(), scale);
            popScissorState();
    ...
    //#if MC>=12111
                emitLine(matrixStack, this.bufferSource.getBuffer(RenderTypes.LINES), line.getLeft(), line.getRight(), state.color, state.lineWidth);
    //#else
