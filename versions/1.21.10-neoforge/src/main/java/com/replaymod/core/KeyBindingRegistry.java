...
public class KeyBindingRegistry extends EventRegistrations {
    private static final KeyBinding.Category CATEGORY = KeyBinding.Category.create(Identifier.of("replaymod", "title"));
    ...
    private Binding registerKeyBinding(String name, int keyCode, boolean onlyInRepay) {
        ...
        if (binding == null) {
            ...
            // Create the KeyBinding; actual registration with NeoForge happens in
            // ReplayModBackend via RegisterKeyMappingsEvent.
            KeyBinding keyBinding = new KeyBinding(key, keyCode, CATEGORY);
