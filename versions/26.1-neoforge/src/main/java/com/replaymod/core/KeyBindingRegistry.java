...
public class KeyBindingRegistry extends EventRegistrations {
    private static final KeyMapping.Category CATEGORY = KeyMapping.Category.register(Identifier.fromNamespaceAndPath("replaymod", "title"));
    ...
    private Binding registerKeyBinding(String name, int keyCode, boolean onlyInRepay) {
        ...
        if (binding == null) {
            ...
            // Create the KeyMapping; actual registration with NeoForge happens in
            // ReplayModBackend via RegisterKeyMappingsEvent.
            KeyMapping keyBinding = new KeyMapping(key, keyCode, CATEGORY);
