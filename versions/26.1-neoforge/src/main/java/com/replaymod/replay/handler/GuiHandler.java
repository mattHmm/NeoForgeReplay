...
public class GuiHandler extends EventRegistrations {
    ...
    private void injectIntoMainMenu(Screen guiScreen, Collection<AbstractWidget> buttonList) {
        ...
        if (mod.getCore().getSettingsRegistry().get(Setting.LEGACY_MAIN_MENU_BUTTON)) {
            ...
            properInjectIntoMainMenu(guiScreen, buttonList instanceof List ? (List<AbstractWidget>) buttonList : new ArrayList<>(buttonList));
        }
        //#else
        //$$ legacyInjectIntoMainMenu(guiScreen, buttonList);
        //#endif
    ...
    //#if MC>=11604
    private void properInjectIntoMainMenu(Screen screen, List<AbstractWidget> buttonList) {
        ...
        if (buttonPosition == MainMenuButtonPosition.BIG) {
            ...
            replayViewerButton = new InjectedButton(
                ...
                //#else
                //$$ protected void renderBackground(PoseStack context, Minecraft client, int mouseX, int mouseY) {
    ...
    private boolean isFancyMenu2Installed() {
        return ModList.get().getModContainerById("fancymenu")
                .map(c -> c.getModInfo().getVersion().toString().startsWith("2."))
                .orElse(false);
    }
    //#endif
            ...
            //#endif
    {
        ...
        public InjectedButton(Screen guiScreen, int buttonId, int x, int y, int width, int height, String buttonText,
            ...
            super(
                    ...
                    //$$ , tooltip != null
                    //$$         ? (button, matrices, mouseX, mouseY) -> guiScreen.renderTooltip(matrices, net.minecraft.network.chat.Component.translatable(tooltip), mouseX, mouseY)
                    //$$         : EMPTY
                    //#endif
                    //#if MC>=11903
        ...
        //#if MC>=12111
        @Override
        protected void extractContents(net.minecraft.client.gui.GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
            // Draw default button label; subclasses override to add icons
            extractDefaultSprite(context);
        }
        //#endif
